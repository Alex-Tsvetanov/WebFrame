...
Coroute utilizes a custom auto-scaffolded CMake system out-of-the-box (e.g., `cmake/CorouteApp.cmake`). Ensuring the structure remains strictly "Modern CMake" guarantees cross-platform reliability when compiling the Dart FFI boundary on Android, iOS, Windows, macOS, and Linux.
...
## 3. Flutter Scaffolding Integrity
- **Preserve `CorouteApp.cmake`:** Do not modify the core flutter scaffold modules unless strictly necessary. Ensure any added directories are safely globbed or explicitly listed before the `add_library(coroute_app SHARED ...)` compilation stage.
- **Transitive Dependencies:** Rely on CMake's transitive dependency resolution. If `coroute_framework` depends on OpenSSL, simply linking `coroute_framework` into your project target will automatically pull in the required OpenSSL include flags and link flags.
