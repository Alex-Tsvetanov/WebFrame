...
TEST_CASE("ResponseBuilder", "[response]") {
	SECTION("basic building") {
		auto resp = ResponseBuilder().status(201).header("X-Custom", "value").body("Created").build();
	...
	SECTION("content_type helper") {
		auto resp = ResponseBuilder().content_type("text/html").body("<h1>Hello</h1>").build();
	...
	SECTION("json_body helper") {
		auto resp = ResponseBuilder().json_body(R"({"status": "ok"})").build();
	...
	SECTION("auto Content-Length") {
		auto resp = ResponseBuilder().body("12345").build();
