...
int main()
{
	...
	app.get("/", [](Request) -> Task<Response> { co_return Response::ok("Hello from HTTP/2 Server!\n"); });
	...
	app.get("/large",
	        ...
	        {
				std::string body;
				body.reserve(1024ULL * 100);  // 100KB
	...
	app.post("/api/data",
	         [](Request req) -> Task<Response>
	         {
				 std::string json = R"({"received":true,"body_size":)" + std::to_string(req.body().size()) + "}";
				 co_return Response::json(json);
			 });

#ifdef COROUTE_HAS_TLS
	// Start HTTPS server with HTTP/2 (h2) on port 8443
	std::cout << "Starting HTTPS server with HTTP/2 on port 8443...\n";
	std::cout << "  Test: curl -k --http2 https://localhost:8443/\n";
	...
	std::cout << "Starting HTTP server with HTTP/2 upgrade (h2c) on port 8080...\n";
	std::cout << "  Test h2c:    curl --http2 http://localhost:8080/\n";
	std::cout << "  Test HTTP/1: curl http://localhost:8080/\n";
