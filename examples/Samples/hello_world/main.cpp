...
int main() {
	...
	app.use(compression({.min_size = 256,  // Compress responses larger than 256 bytes
	                     .level = 6}));
	...
	app.get<size_t, size_t>("/user/{uid}/post/{pid}", [](size_t uid, size_t pid, Request&) -> Task<Response> {
		co_return Response::json(R"({"user_id": )" + std::to_string(uid) + R"(, "post_id": )" + std::to_string(pid) +
		                         "}");
	});
	...
	// POST route example
	app.post("/echo", [](Request& req) -> Task<Response> { co_return Response::ok(std::string(req.body())); });
	...
	app.get("/error", [](Request&) -> Task<Response> { co_return Response::internal_error("Something went wrong!"); });
	...
	app.get("/search", [](Request& req) -> Task<Response> {
		...
		co_return Response::json(R"({"query": ")" + *query + R"(", "page": )" + std::to_string(page) + "}");
	});
	...
	std::cout << "  http://localhost:8080/large  (test compression with: curl -H 'Accept-Encoding: gzip' -v)"
	          << std::endl;
