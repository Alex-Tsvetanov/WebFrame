...
int main(int argc, char* argv[]) {
	...
	// Enable TLS
	app.enable_tls({
	    .cert_file = cert_file, .key_file = key_file, .alpn_protocols = {"http/1.1"}  // Advertise HTTP/1.1 via ALPN
	});
	...
	app.get("/api/data", [](Request&) -> Task<Response> {
		co_return json_response(json_object().set("status", "ok").set("data", json_array({1, 2, 3, 4, 5})).build());
	});
