...
namespace flutter_project::models
{
	...
	{
		return {.username = j.value("username", ""), .password = j.value("password", "")};
	}
	...
	{
		return {
			.username = j.value("username", ""), .email = j.value("email", ""), .password = j.value("password", "")};
	}
