...
namespace flutter_project::handlers::api::users
{
	...
	void register_routes(coroute::App& app, services::UserService& user_service)
	{
		...
		app.post("/api/login",
		         [&user_service](coroute::Request& req) -> coroute::Task<coroute::Response>
		         {
					 try
					 {
						 ...
						 response.add_header("Set-Cookie",
				                             "user_id=" + std::to_string(user->id) + "; Path=/; HttpOnly");
		...
		app.get("/api/users",
		        [&user_service](coroute::Request&) -> coroute::Task<coroute::Response>
		        {
