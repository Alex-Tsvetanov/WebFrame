...
1. **Update `doc/v2/`:** Review and update any relevant markdown documents or architectural overviews inside `doc/v2/` that correspond to the changed feature. Ensure code examples, structural explanations, and endpoint behaviors are perfectly aligned with the new implementation.
...
- No functional change is considered "done" without its corresponding documentation update.
- Agentic task completion must verify that both `doc/v2/` and `README.md` are accurately synchronized before marking the implementation phase complete.
