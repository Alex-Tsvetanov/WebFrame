...
namespace coroute::net
{
	...
	public:
		...
		size_t active_count() const { return active_count_.load(std::memory_order_relaxed); }

		size_t total_acquired() const { return total_acquired_.load(std::memory_order_relaxed); }

		size_t total_reused() const { return total_reused_.load(std::memory_order_relaxed); }
	...
	public:
		...
		PooledConnection(std::unique_ptr<Connection> conn, ConnectionPool* pool) : conn_(std::move(conn)), pool_(pool)
		{
		}
	...
	public:
		explicit AcceptSocketPool(size_t max_pending = 8) : max_pending_(max_pending) { sockets_.reserve(max_pending); }
