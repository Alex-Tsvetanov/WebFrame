...
namespace coroute
{
	...
	public:
		...
		Task<std::optional<Response>> serve_file(const std::filesystem::path& file_path, const Request& req) const;
	...
	private:
		...
		// Build cache headers
		void add_cache_headers(Response& resp, const std::filesystem::path& path, std::uintmax_t size,
		                       std::filesystem::file_time_type mtime) const;

		// Check conditional request (If-None-Match, If-Modified-Since)
		bool check_not_modified(const Request& req, const std::string& etag,
		                        std::filesystem::file_time_type mtime) const;
