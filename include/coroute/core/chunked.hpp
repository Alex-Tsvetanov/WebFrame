...
namespace coroute
{
	...
	public:
		...
		Task<expected<void, Error>> write(const std::string& data) { return write(std::string_view(data)); }
	...
	public:
		...
		const std::vector<std::pair<std::string, std::string>>& trailers() const noexcept { return trailers_; }
	...
	//   });
	Task<Response> streaming_response(net::Connection* conn, std::string content_type, StreamCallback callback);
