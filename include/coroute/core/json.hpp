...
namespace coroute
{
	...
	public:
		...
		// Array access
		JsonValue& operator[](size_t index) { return std::get<JsonArray>(value_)[index]; }
	...
	namespace json
	{
		...
		inline std::string pretty(const JsonValue& value) { return stringify(value, 2); }
	...
	inline expected<JsonValue, Error> parse_json_body(const Request& req) { return json::parse(req); }
	...
	inline JsonBuilder json_object() { return JsonBuilder(); }

	inline JsonArray json_array(std::initializer_list<JsonValue> values) { return JsonArray(values); }
