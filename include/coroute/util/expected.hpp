...
namespace coroute {
	...
	class expected {
		static_assert(!std::is_same_v<T, std::remove_cv_t<unexpected<E> > >, "T cannot be unexpected<E>");
		static_assert(!std::is_same_v<T, unexpect_t>, "T cannot be unexpect_t");

		std::variant<T, unexpected<E> > data_;

		bool has_val() const noexcept { return data_.index() == 0; }
	...
	public:
		...
		constexpr expected(const unexpected<G>& e) : data_(std::in_place_index<1>, unexpected<E>(e.error())) {}

		template <typename G>
		    requires std::is_constructible_v<E, G>
		constexpr expected(unexpected<G>&& e) : data_(std::in_place_index<1>, unexpected<E>(std::move(e.error()))) {}
		...
		template <typename U = T>
		    requires(!std::is_same_v<expected, std::remove_cvref_t<U> >) && std::is_constructible_v<T, U> &&
		...
		constexpr const T* operator->() const noexcept { return std::addressof(std::get<0>(data_)); }
		constexpr T* operator->() noexcept { return std::addressof(std::get<0>(data_)); }

		constexpr const T& operator*() const& noexcept { return std::get<0>(data_); }
		constexpr T& operator*() & noexcept { return std::get<0>(data_); }
		constexpr const T&& operator*() const&& noexcept { return std::move(std::get<0>(data_)); }
		constexpr T&& operator*() && noexcept { return std::move(std::get<0>(data_)); }
		...
		constexpr const E& error() const& noexcept { return std::get<1>(data_).error(); }
		constexpr E& error() & noexcept { return std::get<1>(data_).error(); }
		constexpr const E&& error() const&& noexcept { return std::move(std::get<1>(data_).error()); }
		constexpr E&& error() && noexcept { return std::move(std::get<1>(data_).error()); }

		template <typename U>
		constexpr T value_or(U&& default_value) const& {
			return has_val() ? std::get<0>(data_) : static_cast<T>(std::forward<U>(default_value));
		}
		template <typename U>
		constexpr T value_or(U&& default_value) && {
			return has_val() ? std::move(std::get<0>(data_)) : static_cast<T>(std::forward<U>(default_value));
		}
	...
	public:
		...
		void emplace() noexcept { data_.template emplace<0>(); }
	};
