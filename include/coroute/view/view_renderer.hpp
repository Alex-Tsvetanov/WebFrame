...
namespace coroute {
	...
	struct IViewRenderer {
		...
		virtual void validate_templates(const std::vector<ViewTemplates>& templates) = 0;

		/// Render a view to a Response.
		/// For web, returns Response::html().
		/// For desktop/mobile, implementation may differ.
		virtual Task<Response> render(std::string_view template_id, const ViewResultAny& view_result) = 0;
	};
