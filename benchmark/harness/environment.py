...
def io_backend_from_build(build_dir: Path | None) -> str | None:
    ...
    if build_dir is None:
        return None
    return cache_value(build_dir, "COROUTE_IO_BACKEND")


def cache_value(build_dir: Path | None, key: str) -> str | None:
    """One value out of a build tree's CMakeCache, or None if it cannot be read."""
    if build_dir is None:
        return None
    try:
        text = (Path(build_dir) / "CMakeCache.txt").read_text(
            encoding="utf-8", errors="replace"
        )
    except OSError:
        return None
    match = re.search(rf"^{re.escape(key)}:[A-Z]+=(.*)$", text, re.MULTILINE)
...
# What a compiled artefact can be out of date with respect to. Documentation, the harness
# itself and the thesis are deliberately absent: a night spent refusing campaigns because
# a README moved is a night nobody runs, and a gate that fires for a reason nobody
# believes is a gate someone switches off.
_COMPILED_SOURCES = (
    "src",
    "include",
    "examples",
...
def _ninja_would_rebuild(build_dir: Path, exe: Path) -> bool | None:
    """Whether the build system says this executable is out of date.

    Asking ninja is the point. The alternative is to model the build graph here, and that
    model is wrong in a way rebuilding cannot fix: a change to the generator leaves the
    server's binary genuinely older than a source file, a rebuild correctly does nothing
    for it, and the refusal never clears. ninja already knows which sources reach which
    target, so it answers exactly the question worth asking, per binary.

    Returns None when the tree was not generated by ninja or ninja is absent, and the
    caller falls back to the file comparison.
    """
    if not (build_dir / "build.ninja").exists() or shutil.which("ninja") is None:
        return None
    try:
        target = str(Path(exe).resolve().relative_to(Path(build_dir).resolve()))
    except (ValueError, OSError):
        return None
    out = _run(["ninja", "-C", str(build_dir), "-n", target])
    if out is None:
        return None
    return "no work to do" not in out


def newest_compiled_source(repo: Path) -> tuple[float, Path] | None:
    """The most recently modified compiled source in the tree, and when.

    The file, not the commit that carries it. Comparing a binary against a commit date
    refuses every tree that was built and then committed, which is the ordinary order of
    work and would make the rule something to switch off. Comparing it against the files
    is what a build system already does, and it is right in both directions: a checkout
    or a pull touches exactly the files it changes, so a tree that cannot rebuild shows
    sources newer than its binaries, and a tree that simply has not been committed yet
    does not.
    """
    newest: tuple[float, Path] | None = None
    for entry in _COMPILED_SOURCES:
        path = repo / entry
...
def build_staleness(build_dir: Path | None, repo: Path,
                    executables: Iterable[Path]) -> list[str]:
    """Why the binaries in a build tree do not describe the commit a record will claim.

    Every record carries build.git_commit, and that field describes the source the tree
    is checked out at, never the source the executable was built from. Nothing else in
    the manifest closes the gap: not the compiler, not the backend, not the dependency
    versions. So a tree that cannot rebuild, or was reconfigured and not rebuilt, keeps
    producing records that name a commit its binaries predate, and the file that results
    is indistinguishable from one that was measured properly.

    Both halves are needed and each caught a real case on one machine in one night. A
    binary older than the newest compiled-source commit is the frozen tree: every build
    directory there was pinned to a repository path that no longer existed, so no
    incremental build could succeed and the binaries stayed three days and thirty-five
    commits behind. A binary older than its own CMakeCache is the subtler one: that tree
    was reconfigured at some point and never rebuilt, so its cache and its executable
    described different source states, and anyone reading the cache to learn what the
    binary contained would have been wrong.

    Returns the problems rather than raising, so a caller can name all of them at once.

    The question is put to the build system, which owns the graph and answers it per
    binary; the whole-tree file comparison is only the fallback where ninja cannot be
    asked. That fallback over-refuses, since a change to the generator makes the server
    look old too, and a rebuild does not clear it because the rebuild correctly does
    nothing. A gate that cannot be satisfied is a gate that gets removed, which is why
    it is the fallback and not the rule.

    """
    problems: list[str] = []
    build_dir = Path(build_dir) if build_dir is not None else None
    cache = build_dir / "CMakeCache.txt" if build_dir is not None else None
    cache_mtime = cache.stat().st_mtime if cache is not None and cache.exists() else None
    newest = newest_compiled_source(repo)
    ...
    for exe in executables:
        ...
        if cache_mtime is not None and built < cache_mtime:
            problems.append(
                f"{exe.name} was built before its own CMakeCache.txt was written "
                f"({_stamp(built)} against {_stamp(cache_mtime)}), so the tree was "
                f"configured and not rebuilt and the two describe different source"
            )
        rebuild = _ninja_would_rebuild(build_dir, exe) if build_dir is not None else None
        if rebuild:
            problems.append(
                f"{exe.name} is out of date: the build system reports work still to do "
                f"for it, so it does not contain the source the record would claim"
            )
        elif rebuild is None:
            if newest is None:
                problems.append(
                    f"unchecked: neither the build system nor a compiled source under "
                    f"{repo} could answer whether {exe.name} is current"
                )
            elif built < newest[0]:
                rel = newest[1]
                try:
                    rel = newest[1].relative_to(repo)
                except ValueError:
                    pass
                problems.append(
                    f"{exe.name} is older than {rel} ({_stamp(built)} against "
                    f"{_stamp(newest[0])}), so it may not contain the source the record "
                    f"would claim"
                )
    return problems
...
def capture(repo: Path | None = None, build_type: str | None = None,
            ...
            build_dir: Path | None = None) -> dict[str, Any]:
    ...
    env = {
        ...
        "deps": {
            ...
            # is the only place that knows which one. It was captured nowhere until a
            # tree was found still pinned to a commit that had been the repository's
