...
namespace coroute::http2 {
	...
	Stream::Stream(uint32_t id, Http2Connection* connection, int32_t initial_window_size)
	    : id_(id), connection_(connection), local_window_size_(initial_window_size),
	...
	Task<expected<void, Error>> Stream::send_response(const Response& response) {
		...
		for (const auto& [name, value] : response.headers()) {
			...
			if (lower_name == "connection" || lower_name == "keep-alive" || lower_name == "transfer-encoding" ||
			    lower_name == "upgrade") {
				continue;
			}
	...
	void Stream::transition_to(StreamState new_state) { state_ = new_state; }
	...
	Task<expected<void, Error>> Stream::send_headers(std::span<const Header> headers, bool end_stream) {
	...
	Task<expected<void, Error>> Stream::send_data(std::span<const uint8_t> data, bool end_stream) {
