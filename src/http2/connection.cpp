				...
				while (!end_headers)
					{
								...
								if (!read_result || *read_result == 0)
									{
										co_await send_goaway(ErrorCode::ProtocolError,
										                     "Connection closed pending CONTINUATION");
						...
						if (!cont_header_res)
							{
								co_await send_goaway(ErrorCode::ProtocolError,
								                     "Invalid frame header during CONTINUATION");
						...
						if (cont_header.stream_id != header.stream_id)
							{
								...
								co_return unexpected(
									Error::io(IoError::InvalidArgument, "CONTINUATION stream mismatch"));
							}
						...
						accumulated_headers.insert(accumulated_headers.end(), payload_ptr,
						                           payload_ptr + cont_header.length);
