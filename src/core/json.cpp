...
namespace coroute
{
	...
	namespace
	{
		...
		public:
			...
			expected<JsonValue, Error> parse()
			{
				...
				if (pos_ < input_.size())
				{
					return unexpected(Error::http(HttpError::BadRequest, "Unexpected characters after JSON value"));
				}
				return result;
			}
		...
		private:
			...
			expected<JsonValue, Error> parse_value()
			{
				...
				return unexpected(
					Error::http(HttpError::BadRequest, "Unexpected character in JSON: '" + std::string(1, c) + "'"));
			}
	...
	namespace json
	{
		...
		expected<JsonValue, Error> parse(std::string_view json)
		{
			...
			if (doc.error())
			{
				return unexpected(Error::http(
					HttpError::BadRequest, "JSON parse error: " + std::string(simdjson::error_message(doc.error()))));
			}
			...
			{
				return unexpected(Error::http(HttpError::BadRequest, "JSON parse error: " + std::string(e.what())));
			}
		}
		...
		expected<JsonValue, Error> parse(const Request& req)
		{
			...
			{
				return unexpected(
					Error::http(HttpError::UnsupportedMediaType, "Expected application/json content type"));
			}
			return parse(req.body());
		}

		std::string stringify(const JsonValue& value, int indent) { return value.dump(indent); }
