...
namespace coroute {
	...
	void FormData::add(std::string name, std::string value) { add(FormField{std::move(name), std::move(value)}); }
	...
	namespace form {
		...
		expected<FormData, Error> parse(const Request& req) {
			...
			if (content_type->starts_with("multipart/form-data")) {
				...
				if (!boundary) {
					return unexpected(Error::http(HttpError::BadRequest, "Missing boundary in multipart Content-Type"));
				}
				return parse_multipart(req.body(), *boundary);
			}

			return unexpected(Error::http(HttpError::UnsupportedMediaType, "Unsupported form content type"));
		}
