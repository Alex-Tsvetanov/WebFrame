...
namespace coroute
{
	...
	namespace range
	{
		...
		bool has_range_header(const Request& req) { return req.header("Range").has_value(); }
		...
		bool check_if_range(const Request& req, std::string_view etag, std::string_view last_modified)
		{
	...
	Response partial_content(std::string_view data, int64_t start, int64_t end, int64_t total, std::string content_type)
	{
		Response resp;
		resp.set_status(206);
