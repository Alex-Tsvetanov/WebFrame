...
namespace coroute
{
	...
	namespace
	{
		...
		{
		public:
			const char* name() const noexcept override { return "coroute.io"; }
		...
		{
		public:
			const char* name() const noexcept override { return "coroute.http"; }
				...
				switch (static_cast<HttpError>(ev))
					{
						...
						case HttpError::UriTooLong:
							return "URI Too Long";
						case HttpError::UnsupportedMediaType:
	...
	const std::error_category& io_error_category() noexcept { return io_category_instance; }

	const std::error_category& http_error_category() noexcept { return http_category_instance; }

	std::error_code make_error_code(IoError e) noexcept { return {static_cast<int>(e), io_error_category()}; }

	std::error_code make_error_code(HttpError e) noexcept { return {static_cast<int>(e), http_error_category()}; }
	...
	std::string Error::to_string() const
	{
		...
		if (is_io())
		...
		else if (is_http())
			{
				...
				oss << "HttpError::" << static_cast<int>(e) << " "
					<< http_error_category().message(static_cast<int>(e));
			}
		else if (is_system())
			{
