...
namespace coroute {
	...
	CompressionAlgorithm parse_accept_encoding(std::string_view header, const std::set<CompressionAlgorithm>& enabled) {
		...
		std::sort(candidates.begin(), candidates.end(), [](const EncodingQuality& a, const EncodingQuality& b) {
	...
	namespace compress {
		...
		std::optional<std::string> brotli_decompress(std::string_view data) {
			...
			BrotliDecoderResult ret =
			    BrotliDecoderDecompress(data.size(), reinterpret_cast<const uint8_t*>(data.data()), &output_size,
	...
	CompressionMiddleware::CompressionMiddleware(CompressionOptions options) : options_(std::move(options)) {
	...
	Middleware compression() { return compression(CompressionOptions{}); }
