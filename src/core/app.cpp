															...
															if (proto && *proto == "h2")
																{
																	...
																	auto h2_conn =
																		std::make_shared<http2::Http2Connection>(
																			std::move(*conn_result));
																	h2_conn->set_handler(
																		[this](Request& r) -> Task<Response>
																			{
																				auto match =
																					router_.match(r.method(), r.path());
																				if (match)
																					{
																						r.set_route_params(
																							std::move(match.params));
																					}
																				co_return co_await middleware_chain_
																					.execute_or_not_found(
																						r, match.handler);
																			});
													...
													if (!conn_result)
														{
															if (cancel_source_.is_cancelled()) break;
															std::cerr
																<< "Accept error: " << conn_result.error().to_string()
																<< std::endl;
															continue;
														}
				...
				if (elapsed >= options.drain_timeout)
					{
						std::cout << "Drain timeout reached with "
								  << active_connections_.load(std::memory_order_relaxed) << " connections remaining"
								  << std::endl;
						break;
					}
				...
				if (!req_result)
					{
						if (req_result.error().is_cancelled() ||
						    req_result.error().io_error() == IoError::EndOfStream || req_result.error().is_timeout())
							{
								...
								if (should_close)
								...
								else
									{
										...
										resp.set_header("Keep-Alive", "timeout=30, max=" +
										                                  std::to_string(MAX_REQUESTS_PER_CONNECTION -
										                                                 request_count));
									}
						...
						while (!form_body.empty())
							{
								...
								std::string_view param =
									(amp != std::string_view::npos) ? form_body.substr(0, amp) : form_body;
