...
namespace coroute
{
	...
	Session::Session(std::string id, bool is_new)
		: id_(std::move(id)), created_(std::chrono::system_clock::now()), last_accessed_(created_), is_new_(is_new)
	{
	}
	...
	void Session::touch() { last_accessed_ = std::chrono::system_clock::now(); }
