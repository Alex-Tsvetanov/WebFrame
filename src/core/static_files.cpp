...
namespace coroute
{
	...
	StaticFileServer::StaticFileServer(StaticFileOptions options) : options_(std::move(options))
	{
		// Normalize root path
	...
	void StaticFileServer::add_cache_headers(Response& resp, const std::filesystem::path& path, std::uintmax_t size,
	...
	Task<std::optional<Response>> StaticFileServer::serve_file(const std::filesystem::path& file_path,
