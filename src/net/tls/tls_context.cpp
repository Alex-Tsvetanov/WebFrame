...
namespace coroute::net {
	...
	expected<TlsContext, Error> TlsContext::create(const TlsConfig& config) {
		...
		int min_version = (config.min_version == TlsConfig::MinVersion::TLS_1_3) ? TLS1_3_VERSION : TLS1_2_VERSION;
		SSL_CTX_set_min_proto_version(result.ctx_, min_version);

		// Load certificate
		if (!config.cert_file.empty()) {
			if (SSL_CTX_use_certificate_file(result.ctx_, config.cert_file.string().c_str(), SSL_FILETYPE_PEM) != 1) {
		...
		if (!config.chain_file.empty()) {
			if (SSL_CTX_use_certificate_chain_file(result.ctx_, config.chain_file.string().c_str()) != 1) {
		...
		if (!config.key_file.empty()) {
			if (SSL_CTX_use_PrivateKey_file(result.ctx_, config.key_file.string().c_str(), SSL_FILETYPE_PEM) != 1) {
		...
		if (!config.ca_file.empty()) {
			if (SSL_CTX_load_verify_locations(result.ctx_, config.ca_file.string().c_str(), nullptr) != 1) {
		...
		if (config.verify_client) {
			SSL_CTX_set_verify(result.ctx_, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, nullptr);
		}
	...
	expected<std::unique_ptr<TlsConnection>, Error> TlsConnection::create(std::unique_ptr<Connection> inner,
	                                                                      TlsContext& ctx, bool is_server) {
	...
	Task<TransmitResult> TlsConnection::async_transmit_file(FileHandle file, size_t offset, size_t length) {
	...
	std::optional<std::string_view> TlsConnection::negotiated_protocol() const { return ctx_->alpn_protocol(ssl_); }

	std::string_view TlsConnection::tls_version() const { return SSL_get_version(ssl_); }

	void TlsConnection::set_timeout(std::chrono::milliseconds timeout) {
		if (inner_) inner_->set_timeout(timeout);
	}

	std::string TlsConnection::remote_address() const { return inner_ ? inner_->remote_address() : ""; }

	uint16_t TlsConnection::remote_port() const noexcept { return inner_ ? inner_->remote_port() : 0; }
	...
	TlsListener::TlsListener(std::unique_ptr<Listener> inner, TlsContext& ctx) : inner_(std::move(inner)), ctx_(&ctx) {}
