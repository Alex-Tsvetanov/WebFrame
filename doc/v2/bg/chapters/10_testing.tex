...
\subsection{Тестване на FFI Границите}

Тъй като Coroute се интегрира тясно с Flutter чрез C API, гарантирането на стабилността на FFI границата е от критично значение. За тази цел са разработени специализирани \textit{integration} тестове, които валидират инициализацията, изолирането на изключения и асинхронното предаване на съобщения.

\begin{lstlisting}[language=C++, basicstyle=\small\ttfamily, caption={FFI Bridge тест}]
TEST_CASE("FFI bridge memory request") {
    // 1. Инициализация през C API
    void* engine = coroute_init(R"({"port": 0, "threads": 1})");
    REQUIRE(engine != nullptr);
    
    // 2. Флаг и синхронизация за асинхронния C-callback
    std::atomic<bool> done{false};
    int response_status = 0;
    
    // 3. Извикване на In-Memory Request
    ...
    // 4. Изчакване на отговора
    while (!done) { std::this_thread::sleep_for(10ms); }
    
    REQUIRE(response_status == 200);
    
    // 5. Правилно освобождаване на паметта
    coroute_shutdown(engine);
}
\end{lstlisting}

Този подход гарантира, че управлението на паметта от двете страни на границата (C++ и Dart) остава консистентно и не предизвиква memory leaks или segmentation faults при интензивна комуникация с мобилния интерфейс.
