...
Модулът \textbf{bridge/} предоставя C/C++ FFI границата за експониране на вътрешностите на Coroute към хост среди като Flutter. Този модул дефинира плосък C API интерфейс, изолиращ C++ изключенията и управляващ жизнения цикъл на корутините при комуникация с външни runtime среди.

Модулът \textbf{view/} инкапсулира логиката за сървърно рендиране на шаблони. Включва структури като \texttt{ViewTemplates} за компилация и кеширане на Inja шаблони, \texttt{ViewRouteInfo} за конфигурация на изгледи и интеграция с \texttt{ResponseBuilder} за конструиране на отговори.
...
\subsection{Headless Web Engine и FFI Модел на Изпълнение}

Архитектурата на Coroute не се ограничава само до традиционни мрежови сървъри. Чрез модула \texttt{bridge/}, библиотеката може да функционира като вграден (embedded) headless web engine в крос-платформени клиентски приложения, разработени с инструменти като Flutter.

В този режим на изпълнение, компонентът \texttt{App} не отваря TCP listening socket за външния свят. Вместо това, комуникацията между потребителския интерфейс и уеб двигателя се осъществява локално чрез FFI (Foreign Function Interface) функции.

\subsubsection{C API Изолация}
Тъй като езици като Dart не могат да взаимодействат директно със сложни C++ обекти (особено C++20 корутини като \texttt{Task<T>}), модулът \texttt{bridge/} експортира чист C интерфейс. Всички C++ изключения се прихващат вътрешно на границата и се трансформират в кодове за грешка.

\subsubsection{Вътрешни заявки (Internal Sub-requests)}
Традиционното маршрутизиране се запазва напълно. Вместо да получава HTTP заявка по мрежата, крос-платформеният клиент може директно да извика метода \texttt{App::fetch()}. Този метод генерира изкуствена вътрешна заявка в паметта, прекарва я през същата DFA-базирана маршрутизация, middleware верига и корутинен handler, и връща \texttt{Response} обекта директно като върната стойност. Това гарантира стопроцентово покритие на пътищата за изпълнение (code paths) между самостоятелния сървър и локалното мобилно/десктоп приложение.
