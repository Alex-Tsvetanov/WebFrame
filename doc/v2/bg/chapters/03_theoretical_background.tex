...
\begin{tikzpicture}[scale=0.9]
    ...
    \node[black, font=\small] at (2.5, 3.9) {Заявка 1};
    
    \fill[green!60] (0.3, 2.3) rectangle (4.7, 3.1);
    \node[black, font=\small] at (2.5, 2.7) {Заявка 2};
    
    \fill[orange!60] (0.3, 1.1) rectangle (4.7, 1.9);
    \node[black, font=\small] at (2.5, 1.5) {Заявка 3};
    
    % Arrows showing blocking
    \draw[<-, thick, red] (4.7, 3.9) -- (5.3, 3.9) -- (5.3, 2.8) -- (4.7, 2.8);
    \node[red, font=\tiny, right] at (5.3, 3.3) {чака};
    
    \draw[<-, thick, red] (4.7, 2.6) -- (5.3, 2.6) -- (5.3, 1.5) -- (4.7, 1.5);
    \node[red, font=\tiny, right] at (5.3, 2.1) {чака};
    ...
    \draw[thick] (6.5, 0) rectangle (14.5, 5);
...
\begin{tikzpicture}[scale=0.85]
    ...
    \node[font=\small, align=left, fill=white, opacity=0.95, text opacity=1] at (5, -1.5) {
        \begin{tabular}{l}
        GET /chat HTTP/1.1 \\
        Upgrade: websocket \\
        Connection: Upgrade \\
    ...
    \node[font=\small, align=left, fill=white, opacity=0.95, text opacity=1] at (5, -4.5) {
        \begin{tabular}{l}
...
\begin{tikzpicture}[scale=0.8]
    ...
    \node[font=\footnotesize, fill=blue!20, draw, rounded corners] at (6, -1.25) {
        \begin{tabular}{c}
        ClientHello \\
    ...
    \node[font=\footnotesize, fill=green!10, draw, rounded corners] at (6, -3.45) {
        \begin{tabular}{c}
    ...
    \node[font=\footnotesize, fill=green!10, draw, rounded corners] at (6, -5.5) {
        \begin{tabular}{c}
        EncryptedExtensions \\
    ...
    \node[font=\footnotesize, fill=blue!10, draw, rounded corners] at (6, -7.25) {Finished};
...
В Coroute, имплементацията на IOCP използва няколко ключови техники за максимална производителност. За трансфер на файлове се използва нейтив функцията \texttt{TransmitFile}, която позволява директен zero-copy трансфер от файловата система към мрежовия сокет. Използва се и механизма на \texttt{OVERLAPPED} структурите за ефективно проследяване на състоянията на асинхронните операции, което премахва нуждата от сложна синхронизация при обработка на завършените събития.
...
Имплементацията на io\_uring в Coroute е проектирана за изключителна производителност чрез архитектура с \textbf{per-thread rings}. Всяка нишка разполага със собствен io\_uring инстанс, което елиминира конкуренцията за заключвания (lock contention) между нишките. При работа като сървър се използва \texttt{SO\_REUSEPORT} за балансиране на входящите връзки директно от ядрото между отделните нишки (\textbf{multi-accept}). Обработката на завършените операции се извършва на партиди (\textbf{batching}), като се използва нейтив функцията \texttt{sendfile} за zero-copy производителност при работа с файлове.
...
kqueue също поддържа edge-triggered и level-triggered режими, подобно на epoll. В edge-triggered режим, събитие се докладва само когато състоянието се промени. В level-triggered режим, събитие се докладва докато условието е изпълнено.

В Coroute, имплементацията на kqueue е оптимизирана за максимална производителност чрез няколко ключови техники. Първо, използва се \textbf{syscall batching}, при който множество регистрации на събития се натрупват в thread-local буфер и се подават на ядрото заедно с извикването за изчакване на нови събития. Това елиминира излишните системни извиквания за всяка асинхронна операция. Второ, използван е \textbf{lock-free} подход за проследяване на чакащите операции чрез \texttt{udata} полето на \texttt{kevent} структурите, което премахва нуждата от глобални заключвания при обработка на събития. Трето, за пренос на файлове се използва нейтив функцията \texttt{sendfile}, която осигурява zero-copy трансфер на данни директно от файловата система към мрежовия сокет.

Фигура \ref{fig:io-comparison} сравнява архитектурите на IOCP, kqueue и io\_uring.
...
\begin{tikzpicture}[scale=0.55]
    % IOCP Side
    \node[font=\bfseries] at (2.5, 8.5) {IOCP (Windows)};
    
    % Application
    \node[draw, thick, fill=blue!20, minimum width=4cm, minimum height=0.7cm] (app1) at (2.5, 6.5) {Приложение};
    
    % Worker threads
    \node[draw, fill=green!20, minimum width=0.8cm, minimum height=0.5cm, font=\tiny] (t1) at (0, 4.5) {Thread 1};
    \node[draw, fill=green!20, minimum width=0.8cm, minimum height=0.5cm, font=\tiny] (t2) at (2.5, 4.5) {Thread 2};
    \node[draw, fill=green!20, minimum width=0.8cm, minimum height=0.5cm, font=\tiny] (t3) at (5, 4.5) {Thread N};
    
    % Completion Port
    \node[draw, thick, fill=orange!30, minimum width=3.5cm, minimum height=0.8cm] (cp) at (2.5, 2.5) {Completion Port};
    
    % Kernel
    \node[draw, thick, fill=gray!20, minimum width=4cm, minimum height=1cm] (kernel1) at (2.5, 0) {Windows Kernel};
...
\begin{tikzpicture}[scale=0.55]
    % kqueue Side
    \node[font=\bfseries] at (2.5, 8.5) {kqueue (macOS/BSD)};
    
    % Application
    \node[draw, thick, fill=blue!20, minimum width=4cm, minimum height=0.7cm] (app3) at (2.5, 6.5) {Приложение};
    
    % Buffer
    \node[draw, fill=yellow!20, minimum width=3.5cm, minimum height=0.5cm, font=\tiny] (buf) at (2.5, 4.5) {Batch Buffer};
    
    % kevent
    \node[draw, thick, fill=orange!30, minimum width=3.5cm, minimum height=0.8cm] (kq) at (2.5, 2.5) {kevent() Queue};
    
    % Kernel
    \node[draw, thick, fill=gray!20, minimum width=4cm, minimum height=1cm] (kernel3) at (2.5, 0) {macOS Kernel};
    
    % Arrows
    \draw[->, thick] (app3) -- (buf);
    \draw[->, thick] (buf) -- (kq);
    \draw[->, thick] (kq) -- (kernel3);
    \draw[->, thick] (kernel3) -- (kq);
    \draw[->, thick] (kq.west) -- ++(-1, 0) |- (app3.west);
\end{tikzpicture}
\caption{kqueue: Event-based}
\label{fig:io-kqueue}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
...
\begin{tikzpicture}[scale=0.55]
    % io_uring Side
    \node[font=\bfseries] at (2.5, 8.5) {io\_uring (Linux)};
    
    % Application
    \node[draw, thick, fill=blue!20, minimum width=4cm, minimum height=0.7cm] (app2) at (2.5, 6.5) {Приложение};
    
    % Ring buffers
    \node[draw, fill=yellow!30, minimum width=1.5cm, minimum height=0.7cm, font=\tiny] (sq) at (0.25, 3.5) {SQ};
    \node[draw, fill=purple!30, minimum width=1.5cm, minimum height=0.7cm, font=\tiny] (cq) at (4.75, 3.5) {CQ};
    
    % Shared memory indicator
    \draw[<->, dashed, thick, red] (sq) -- (cq);
    
    % Kernel
    \node[draw, thick, fill=gray!20, minimum width=4cm, minimum height=1cm] (kernel2) at (2.5, 0) {Linux Kernel};
    
    % Arrows
    \draw[->, thick, blue] (app2) -- (sq);
    \draw[->, thick, green!60!black] (cq) -- (app2);
    \draw[->, thick] (sq) -- (kernel2);
    \draw[->, thick] (kernel2) -- (cq);
\end{tikzpicture}
\caption{io\_uring: Ring buffers}
\label{fig:io-iouring}
\end{subfigure}
\caption{Сравнение на архитектурите на асинхронния I/O при различните платформи}
...
В съвременната софтуерна разработка често възниква необходимост от комбиниране на код, написан на различни езици за програмиране. Механизмът, който позволява на програма, написана на един език, да извиква подпрограми или да използва услуги, написани на друг език, се нарича Foreign Function Interface (FFI).

Исторически, C езикът се е утвърдил като de facto стандарт за системна оперативна съвместимост. Почти всички съвременни езици от високо ниво (Python, Java, Dart, Rust, Go) предоставят възможност за извикване на C функции чрез съответните FFI имплементации (напр. JNI за Java, \texttt{ctypes} за Python, или \texttt{dart:ffi} за Dart). Тъй като C++ поддържа експортиране на функции със C свързаност (чрез ключовата дума \texttt{extern "C"}), той може напълно да се интегрира в тези екосистеми.

\subsubsection{Паметови граници и управление}

При преминаване на границата между два езика чрез FFI, основното предизвикателство е управлението на паметта. Езиците с автоматично събиране на боклука (Garbage Collection), като Dart, имат строги правила за локация и преместване на обекти в паметта. C++, от друга страна, очаква ръчно управление на паметта. Затова данните, които се предават през FFI, обикновено изискват алокиране на непрекъснати блокове памет извън хийпа на garbage collector-а или внимателно синхронизиране на жизнения цикъл на обектите.

\subsubsection{Контекст на изпълнение и асинхронност}

Извикването на C/C++ функция от език на по-високо ниво обикновено блокира текущата нишка до завършване на функцията. Това е проблематично в среди като Dart (Flutter), където потребителският интерфейс работи в единна основна нишка (isolate). За да не блокира UI-а, дългите операции в C++ трябва да се изпълняват в системни или background нишки. Възниква необходимост от механизъм за обратна връзка (callbacks), чрез който C++ може асинхронно да уведоми хост езика за завършване на операция, подавайки съобщение обратно през границата на езиците. Този модел е ключов за изграждане на "headless" двигатели, където бизнес логиката и I/O операциите се изпълняват ефективно от C++, докато фронтендът само реагира на събития.

\subsection{Шаблонни машини (Template Engines)}

Шаблонните машини са софтуерни компоненти, които комбинират статичен шаблон с динамични данни за генериране на крайни документи (обикновено HTML, XML или други текстови формати). В контекста на уеб приложенията, те са фундаментална част от архитектурата Server-Side Rendering (SSR).

Разделянето на презентационния слой от бизнес логиката е основен принцип в шаблонизирането. Шаблонът обикновено съдържа крайния маркъп, в който са вмъкнати специални тагове или директиви за контрол (напр. цикли, условни конструкции, променливи). По време на изпълнение, шаблонната машина парсва тези файлове, оценява контролните директиви спрямо подаден контекст (речник с данни) и синтезира крайния низ.

Имплементации като Inja (C++ имплементация, базирана на популярния Jinja синтаксис за Python), използват JSON като формат за подаване на контекстуални данни. Интеграцията им директно в уеб сървъра позволява генерирането на персонализирани отговори без необходимост от допълнителни мрежови заявки от клиента към API, което намалява латентността при първоначално зареждане.
