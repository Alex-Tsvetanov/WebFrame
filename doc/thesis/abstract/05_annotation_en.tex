...
counts. The generator's pacing lag is recorded and reported beside every cell but admits nothing:
it is partly a property of the server it would judge, so the one-millisecond limit on it was
removed and the stored records were re-evaluated rather than re-run, which returned
\R{campaign.x1.readmitted} runs to the sample.
