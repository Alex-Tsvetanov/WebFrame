...
% --- Language ------------------------------------------------------------
% Both languages are loaded whichever way lang.tex is set; only the main one moves.
% That keeps T2A and the Cyrillic fonts live in the English build, so the title page
% and quoted terminology can still use \foreignlanguage{bulgarian}{...}.
\ifbg
...
% \bgen{bulgarian}{english} picks a string for the current language.
%
% Defined by branching HERE, at load time, rather than as
%   \newcommand{\bgen}[2]{\ifbg#1\else#2\fi}
% The second form leaves a live conditional inside the macro body, which breaks when
% the macro is expanded during a write into .toc or .aux. This form is fully
% expandable and is therefore safe inside \MakeUppercase and \addcontentsline.
\ifbg
  \newcommand{\bgen}[2]{#1}
\else
  \newcommand{\bgen}[2]{#2}
...
\DeclareCaptionLabelFormat{tablelabel}{\bgen{Таблица}{Table} #2}
...
\DeclareCaptionLabelFormat{figurelabel}{\bgen{Фигура}{Figure} #2}
...
\newcommand{\tabref}[1]{(\bgen{Табл.}{Table}~\ref{#1})}
\newcommand{\figref}[1]{(\bgen{Фиг.}{Fig.}~\ref{#1})}
...
% Диаграмите се четат и на черно-бял печат, затова се разчита на форма, щриховка и
% дебелина на линията, а не на цвят. Стиловете са събрани тук, за да изглеждат всички
% фигури като части от един документ, а не като единайсет отделни рисунки.
\tikzset{
...
% figures/ съдържа по една фигура на файл. Вмъкват се на мястото, на което текстът
% ги обсъжда, а не се събират в приложение: фигура, до която се стига по препратка,
% се пропуска при четене.
\newcommand{\figdir}{figures}

% data/ съдържа CSV файловете, произведени от кампанията. Таблиците и графиките в
% глава VI ги четат направо, тоест нито едно число в тях не минава през човек.
...
% --- Числа и графики от измервания ---------------------------------------
% siunitx: един формат за числата в целия документ, вместо решение на всяко
% изречение. pgfplots и pgfplotstable четат CSV направо, така че нито една
% фигура или таблица с резултати не се копира на ръка.
\usepackage{siunitx}
% Без закръгляване тук. Значещите цифри зависят от величината: пропускателната
% способност е цяло число, латентността е с два знака, а брой изпълнения няма
% дробна част изобщо. Инструментът, който знае какво означава числото, го
% закръглява при генерирането; siunitx отговаря само за вида.
\sisetup{group-separator={\,}, group-minimum-digits=4, round-mode=none}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
\usepackage{pgfplotstable}
% Един вид за всички графики в глава VI, за да изглеждат като части от
% един документ, а не като поредица от отделни картинки.
\pgfplotsset{
  thesis plot/.style={
...
\makeatother
\renewcommand{\lstlistingname}{\bgen{Листинг}{Listing}}
\renewcommand{\lstlistlistingname}{\bgen{Списък на листингите}{List of Listings}}
...
% Стойности от измерванията, генерирани от tools/results2tex.py.
%
% Числата в изреченията се пишат като \R{ключ}, а не на ръка. Липсваща стойност се
% отпечатва като червено [?ключ] и се намира с grep, вместо да остане правдоподобно
% изглеждащо число, което никой не е проверил. Файлът се \input-ва само ако съществува,
% за да се компилира документът и на машина, на която не са пускани измервания.
\providecommand{\R}[1]{\textcolor{red}{\textbf{[?#1]}}}
\IfFileExists{generated/results.tex}{\input{generated/results}}{}

% Таблица или графика, чийто CSV още не е произведен, се отпечатва като същия червен
% маркер вместо да спре компилацията: pgfplots третира липсващ файл като фатална грешка,
% а документът трябва да се строи и на машина, на която не са пускани измервания.
\newcommand{\missingdata}[1]{\begin{center}\textcolor{red}{\textbf{[?\code{\datadir/\detokenize{#1}}]}}\end{center}}
