...
\begin{tikzpicture}[
  ...
  % === пояснения =============================================================
  % Текстът е разбит на къси редове, за да не опре в спускането на HTTP/3 вдясно.
  \node[note, anchor=north west] at (5.9,3.75)
  ...
  \node[fbox, fill=gray!25, thick, minimum width=4.6cm] (fa) at (2.1,-2.35)
    {предложение\\[2pt]
     $L_{\text{demux}} = W \cdot |T|$\\[2pt]
     {\fontsize{8}{9.5}\selectfont броят не зависи от $|P|$}};
  \node[fbox, fill=gray!10, minimum width=7.8cm] (fb) at (8.7,-2.35)
...
\end{tikzpicture}
\caption{Брой на слушащите дескриптори при класификация след приемане на връзката. Всяка
работна нишка притежава по един слушащ дескриптор за \code{TCP} и по един за \code{UDP}.
...
броенето. Остава $L_{\text{demux}} = W \cdot |T|$. Вдясно долу е обичайната конфигурация за
