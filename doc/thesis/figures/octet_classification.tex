...
\begin{tikzpicture}[
    ...
    lbl/.style={font=\fontsize{8}{9.5}\selectfont, align=left, inner sep=1.5pt,
                fill=white},
    % Бележките локално са с по-дребен шрифт, за да са в мащаба на кутиите.
    note/.append style={font=\fontsize{8}{10}\selectfont, inner sep=4pt},
  ]
  ...
  \node[note, anchor=north, text width=3.0cm] at (13.0,4.45)
  ...
  \node[note, anchor=south east] at (7.0,4.60) {\code{Maybe}: още байтове};
  ...
  \node[note, anchor=north, text width=2.7cm] at (6.9,1.35)
    {Предварително знание, без \code{ALPN}.};
  \node[note, anchor=north, text width=2.7cm] at (10.15,1.35)
    {Надграждане до \code{WebSocket} или \code{h2c}.};

  % ---- твърдението, върху което стъпва решението с един байт ---------------
  \node[grp, note, text width=13.0cm, fill=gray!10] at (6.85,-0.6)
